...
为了保证项目的开发的一致性和质量，AI 代理在协作时需遵循以下规则：

1. **提交日志**: 生成提交日志时请使用 **中文**，详细描述本次修改实现的功能。
2. **前端开发**: 修改前端 JS、Vue 等文件时，需通过 **ESLint** 规则验证。
3. **进度记录**: 每次提交后请及时同步更新 [DEVELOPMENT_LOG.md](file:///d:/MyCodes/OtherCodes/simple-boot-mock-server07/DEVELOPMENT_LOG.md)。
4. **核心文档**: 发生重大功能变更或架构调整时，请务必更新 [AGENT.md](file:///d:/MyCodes/OtherCodes/simple-boot-mock-server07/AGENT.md)。
