...
const loadUserGroups = async () => {
  ...
  try {
    const res = await DashboardApi.getTopUserGroups(10, all.value)
    if (res && res.success) {
...
const computedOption = computed(() => {
  ...
  return {
    ...
    series: [
      {
        name: $i18nBundle('mock.label.mockGroupCount'),
        type: 'bar',
        barWidth: '40%',
        itemStyle: {
          color: '#E6A23C', // Warning color to distinguish from UserCalls
  ...
  >
    <template #header>
      <div class="card-header">
        <span class="card-title">{{ $t('mock.label.topContributors') }}</span>
      </div>
    </template>
    <v-chart
      class="chart"
